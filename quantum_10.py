#!/usr/bin/env python3
"""
'Quantum-simulated' solver for a 10-link constant-curvature robot
...
def build_10link_system_physical_nodamping(n=30, T=10.0):
    """
    Build pseudospectral system L x_vec = b_vec for a 10-link model with:
...
if __name__ == "__main__":
    ...
    # Build system (10 links)
    L, b, t_nodes, d, A_global, f, tau_phi, J, L_link = \
        build_10link_system_physical_nodamping(n=n, T=T)
    N_links = d // 2
    ...
    try:
        ...
        plt.title("Angle Error per Link (Quantum − Classical, 10 links)")
        plt.grid(True)
        ...
        plt.title("Angular Velocity Error per Link (Quantum − Classical, 10 links)")
        plt.grid(True)
        plt.legend(fontsize=8, ncol=3)
        plt.tight_layout()
        ...
        def update(frame_idx):
            ...
            title_anim.set_text(f"Link motion (no damping, 10 links), t = {t_f:.1f} s")
        ...
        ani.save("link_motion_classical_vs_quantum_10links.gif", writer=gif_writer)
