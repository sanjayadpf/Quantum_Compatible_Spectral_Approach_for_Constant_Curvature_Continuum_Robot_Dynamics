#!/usr/bin/env python3
"""
'Quantum-simulated' solver for a 25-link constant-curvature robot
matching the classical script (no damping), WITHOUT using Qiskit.

Same parameters per link as classical script:
    n = 30
    T = 10.0 s
...
Process:
    ...
    - Compare classical vs 'quantum-simulated' solutions
      and:
        * plot error in phi_i, phi_dot_i for all links
...
def build_25link_system_physical_nodamping(n=30, T=10.0):
    """
    Build pseudospectral system L x_vec = b_vec for a 25-link model with:
...
# ==============================================================
# Forward kinematics for planar chain (any number of links)
# ==============================================================

def forward_kinematics_chain(phi_vec, L_link):
    """
    Given phi_vec = [phi_1,...,phi_N] and link length L_link,
    compute joint positions (x,y) from base to tip (N+1 points).
    """
    N_links = len(phi_vec)
    xs = [0.0]
    ys = [0.0]
...
if __name__ == "__main__":
    ...
    # Build system (25 links)
    L, b, t_nodes, d, A_global, f, tau_phi, J, L_link = \
        build_25link_system_physical_nodamping(n=n, T=T)
    N_links = d // 2
    ...
    print("Number of links N_links =", N_links)
    ...
    print("\nFinal state x(T=10) for all links:")
    for i in range(N_links):
    ...
    Phi_classical = X_classical[:, 0::2]   # (n+1, N_links)
    Phi_quantum = X_quantum[:, 0::2]
    Phi_err = Phi_quantum - Phi_classical  # (n+1, N_links)
    ...
    try:
        ...
        # Error plots only
        # -----------------------------
        # Angle error Δphi_i(t) for all links
        plt.figure(figsize=(8, 5))
        for i in range(N_links):
            plt.plot(t_nodes, Phi_err[:, i], label=f"Link {i+1}")
        plt.xlabel("Time t (s)")
        plt.ylabel("Angle error Δphi_i (rad)")
        plt.title("Angle Error per Link (Quantum − Classical, 25 links)")
        plt.grid(True)
        plt.legend(fontsize=6, ncol=3)
        plt.tight_layout()

        # Angular velocity error Δphi_dot_i(t) for all links
        plt.figure(figsize=(8, 5))
        for i in range(N_links):
            plt.plot(t_nodes, PhiDot_err[:, i], label=f"Link {i+1}")
        plt.xlabel("Time t (s)")
        plt.ylabel("Angular velocity error Δphi_dot_i (rad/s)")
        plt.title("Angular Velocity Error per Link (Quantum − Classical, 25 links)")
        plt.grid(True)
        plt.legend(fontsize=6, ncol=3)
        plt.tight_layout()
        ...
        xs0_c, ys0_c = forward_kinematics_chain(phi0_c, L_link)
        xs0_q, ys0_q = forward_kinematics_chain(phi0_q, L_link)
        ...
        def update(frame_idx):
            ...
            xs_c, ys_c = forward_kinematics_chain(phi_c, L_link)
            xs_q, ys_q = forward_kinematics_chain(phi_q, L_link)
            line_c.set_data(xs_c, ys_c)
            line_q.set_data(xs_q, ys_q)
            title_anim.set_text(f"Link motion (no damping, 25 links), t = {t_f:.1f} s")
        ...
        ani.save("link_motion_classical_vs_quantum_25links.gif", writer=gif_writer)
